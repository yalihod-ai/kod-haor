...
var tiles = [
  ...
  {ic:'🧪', title:'בנק המתכונים', desc:'28 מתכונים מעולים נוספים להרחבת שגרת הטיפוח שלך: לפנים, גוף ואמבט, כדי שיהיו לך עוד אפשרויות הכנה בבית.', href:'https://yalihodschool.co.il/kod-haor-recipes/', need:'recipes', buy:'https://secure.cardcom.solutions/EA/EA5/pjs2vZDJN0iGXaci7MbvWA/PaymentSP'},
...
(function(){
  try{
    var st = window.KH && window.KH.state;
    if(!st) return;
    var canRedo = window.KH.canRedo === true;
    var left    = window.KH.redoInDays || 0;
    var at      = window.KH.skinAt ? new Date(window.KH.skinAt*1000) : null;
    var when    = at ? (at.getDate()+'.'+(at.getMonth()+1)+'.'+at.getFullYear()) : '';
    var grid = document.getElementById('grid');
    if(!grid) return;
    var d = document.createElement('div');
    if(canRedo){
      d.setAttribute('style','background:linear-gradient(135deg,#a98c50,#c2a66e);color:#fff;border-radius:16px;padding:20px 24px;margin-bottom:20px;text-align:center;box-shadow:0 10px 30px rgba(169,140,80,.3)');
      d.innerHTML = '<div style="font-size:1.1rem;font-weight:800;margin-bottom:6px">✨ אפשר לעשות אבחון מחדש</div>'
        + '<div style="opacity:.92;font-size:.95rem;margin-bottom:14px">עברה חצי שנה מהאבחון שלך' + (when? ' ('+when+')':'') + '. העור משתנה — שווה לבדוק מחדש מה הוא צריך עכשיו.</div>'
        + '<a href="https://yalihodschool.co.il/kod-haor-diagnosis/" style="display:inline-block;background:#fff;color:#a98c50;font-weight:800;padding:12px 30px;border-radius:99px;text-decoration:none">לאבחון מחדש ←</a>';
    } else {
      d.setAttribute('style','background:#faf7f0;border:1px solid #e5dfce;border-radius:14px;padding:14px 18px;margin-bottom:20px;text-align:center;color:#8a7154;font-size:.92rem');
      d.innerHTML = 'האבחון שלך נעשה' + (when? ' ב-'+when : '') + ' · אפשר יהיה לעשות אבחון מחדש בעוד <b>' + left + '</b> ימים';
    }
